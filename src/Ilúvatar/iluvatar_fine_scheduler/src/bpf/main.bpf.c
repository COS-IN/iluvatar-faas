...
void BPF_STRUCT_OPS(rustland_running, struct task_struct *p)
{
	...
	dbg_msg("start: pid=%d (%s) cpu=%ld holding_cpu=%ld start_time=%ld utime=%ld stime=%ld gtime=%ld", 
            p->pid, 
            p->comm, 
            cpu, 
            p->scx.holding_cpu,
...
void BPF_STRUCT_OPS(rustland_stopping, struct task_struct *p, bool runnable)
{
	...
	dbg_msg("stop: pid=%d (%s) cpu=%ld holding_cpu=%ld", 
            p->pid, 
            p->comm, 
            cpu,
            p->scx.holding_cpu
				...
				struct scx_cpu_release_args *args)
{
	 ...
	 */
	dbg_msg("[updated] cpu preemption: pid=%d (%s)", p->pid, p->comm);
	if (is_usersched_task(p))
		set_usersched_needed();
}
