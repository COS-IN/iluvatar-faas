...
use anyhow::Result;
use crate::{transaction::TransactionId, nproc, bail_error, threading};
...
impl CPUService {
  ...
  fn read_freq(&self, pth: PathBuf, tid: &TransactionId) -> Result<u64> {
    let mut opened = match File::open(&pth) {
      Ok(b) => b,
      Err(e) => bail_error!(error=%e, tid=%tid, file=%pth.to_string_lossy(), "Unable to open cpu freq file"),
    };
    let mut buff = String::new();
    match opened.read_to_string(&mut buff) {
      Ok(_) => (),
      Err(e) => bail_error!(error=%e, tid=%tid, file=%pth.to_string_lossy(), "Unable to read cpu freq file into buffer"),
    };
    match buff[0..buff.len()-1].parse::<u64>() {
      Ok(u) => Ok(u),
      Err(e) => bail_error!(error=%e, tid=%tid, data=%buff, "Unable to parse cpu freq buffer"),
    }
  }
  ...
  pub fn boxed(kernel_freq_ms: Option<u64>, hardware_freq_ms: Option<u64>, tid: &TransactionId) -> Result<Arc<Self>> {
    ...
    let kernel = match kernel_freq_ms {
      ...
      Some(ms) => {
        ...
        let (h, t) = threading::os_thread(ms, KERNEL_FREQ_WORKER_TID.clone(), Arc::new(Self::kernel_frequencies))?;
        (Some(h), Some(t))
      },
    };
    let hardware = match hardware_freq_ms {
      ...
      Some(ms) => {
        ...
        let (h, t) = threading::os_thread(ms, HARDWARE_FREQ_WORKER_TID.clone(), Arc::new(Self::hardware_frequencies))?;
        (Some(h), Some(t))
      },
    };
