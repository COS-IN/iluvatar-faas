...
pub fn start_perf_stat<S>(outfile: &S, tid: &TransactionId, stat_duration_sec: u64) -> anyhow::Result<Child> 
  where S: AsRef<str> + ?Sized + std::fmt::Display {

  let dur = (stat_duration_sec * 1000).to_string();
  let args = vec!["stat", "-e", "power/energy-pkg/", "-I", &dur.as_str(), "-x", ",", "--output", outfile.as_ref()];
