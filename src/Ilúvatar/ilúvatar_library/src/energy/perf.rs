...
pub fn start_perf_stat<S>(outfile: &S, tid: &TransactionId, stat_duration_ms: u64) -> anyhow::Result<Child> 
  where S: AsRef<str> + ?Sized + std::fmt::Display {

  let st = stat_duration_ms.to_string();
  let args = vec!["stat", "-e", "power/energy-pkg/", "-I", &st.as_str(), "-x", ",", "--output", outfile.as_ref()];
