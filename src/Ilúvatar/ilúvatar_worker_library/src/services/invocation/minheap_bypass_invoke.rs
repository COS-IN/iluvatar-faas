...
#[derive(Debug)]
pub struct MHQBPEnqueuedInvocation {
    x: Arc<EnqueuedInvocation>,
    exectime: f64
}

impl MHQBPEnqueuedInvocation {
    fn new( x: Arc<EnqueuedInvocation>, exectime: f64 ) -> Self {
        MHQBPEnqueuedInvocation {
            x,
            exectime
        }
    }
}
impl Eq for MHQBPEnqueuedInvocation {
}
impl Ord for MHQBPEnqueuedInvocation {
 fn cmp(&self, other: &Self) -> Ordering {
    compare_f64( &self.exectime, &other.exectime ).reverse()
 }
}

impl PartialOrd for MHQBPEnqueuedInvocation {
 fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
     Some(compare_f64( &self.exectime, &other.exectime ).reverse())
 }
}

impl PartialEq for MHQBPEnqueuedInvocation {
...
pub struct MinHeapBPInvoker {
  ...
  pub invoke_queue: Arc<Mutex<BinaryHeap<Arc<MHQBPEnqueuedInvocation>>>>,
...
impl Invoker for MinHeapBPInvoker {
  ...
  fn add_item_to_queue(&self, item: &Arc<EnqueuedInvocation>, _index: Option<usize>) {
    ...
    queue.push(MHQBPEnqueuedInvocation::new(item.clone(), self.cmap.get_exec_time(&item.fqdn)).into());
