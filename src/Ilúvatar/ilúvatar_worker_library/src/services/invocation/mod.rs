...
use self::{minheap_ed_invoke::MinHeapEDInvoker, fcfs_bypass_invoke::FCFSBypassInvoker, minheap_bypass_invoke::MinHeapBPInvoker, minheap_ed_bypass_invoke::MinHeapEDBPInvoker };
...
impl InvokerFactory {
  ...
  pub fn get_invoker_service(&self, tid: &TransactionId) -> Result<Arc<dyn Invoker>> {
    let r: Arc<dyn Invoker> = match self.invocation_config.queue_policy.to_lowercase().as_str() {
      ...
      "minheap_ed_bypass" => {
        MinHeapEDBPInvoker::new(self.cont_manager.clone(), self.function_config.clone(), self.invocation_config.clone(), tid, self.cmap.clone())?
      },
      "minheap_bypass" => {
