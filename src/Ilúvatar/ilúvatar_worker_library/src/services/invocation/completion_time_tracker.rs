...
mod tracker_tests {
  use super::*;
  use more_asserts::{assert_le, assert_gt, assert_lt};
  use rand::Rng;
  ...
  fn added_items_ordered() {
    ...
    for i in 0..items.len() {
      if i < items.len()-1 {
        assert_lt!(items[i], items[i+1], "Items were out of order: {:?}", *tracker.items.read());
      }
    }
  }
  ...
  fn random_insertions_ordered() {
    ...
    for i in 0..items.len() {
      if i < items.len()-1 {
        assert_le!(items[i], items[i+1], "Items were out of order: {:?}", *tracker.items.read());
      }
    }
  }
