...
impl DockerIsolation {
  ...
  fn parse_mem(input: std::borrow::Cow<str>) -> Result<MemSizeMb> {
    ...
    if end_of_num > input.len() {
      anyhow::bail!("End of scale {} is somehow greater that input '{}'", end_of_scale, input)
    }
...
impl ContainerIsolationService for DockerIsolation {
  ...
  async fn clean_containers(&self, _ctd_namespace: &str, _self_src: Arc<dyn ContainerIsolationService>, tid: &TransactionId) -> Result<()> {
...
mod docker_tests {
  ...
  fn parse_mem_works(#[case] input: &str, #[case] expected: MemSizeMb) {
    assert_eq!(DockerIsolation::parse_mem(std::borrow::Cow::Borrowed(input)).unwrap(), expected);
  }
