...
use crate::utils::{sim_invoker_svc, sim_args, test_invoke};
...
mod container_available {
  ...
  async fn cpu_prewarm_available_after_prewarm() {
    ...
    assert_eq!(cm.container_available(&func.fqdn, Compute::CPU), ContainerState::Prewarm);
  }
  ...
  async fn gpu_prewarm_available_after_prewarm() {
    ...
    assert_eq!(cm.container_available(&func.fqdn, Compute::GPU), ContainerState::Prewarm);
  }
