...
use self::{containers::{structs::{Container, RegisteredFunction}, ContainerdLifecycle}, network::namespace_manager::NamespaceManager};
...
pub trait LifecycleService: Send + Sync + std::fmt::Debug {
  ...
  async fn remove_container(&self, container_id: Arc<Container>, ctd_namespace: &str, tid: &TransactionId) -> Result<()>;
