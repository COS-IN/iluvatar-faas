use std::sync::Arc;
use crate::{ilúvatar_api::WorkerAPI, rpc::RPCWorkerAPI, load_balancer_api::structs::internal::RegisteredWorker, bail_error, transaction::TransactionId, worker_api::create_worker};
use anyhow::Result;
use dashmap::DashMap;
use super::{sim_worker::SimWorkerAPI, ilúvatar_worker::IluvatarWorkerImpl};
  ...
  pub async fn get_worker_api(&self, worker: &Arc<RegisteredWorker>, tid: &TransactionId) -> Result<Box<dyn WorkerAPI + Send>> {
    if worker.communication_method == "RPC" {
      ...
      let api = match self.try_get_simapi(worker) {
        Some(api) => api,
        None => {
          let worker_config = crate::worker_api::worker_config::Configuration::boxed(false, &worker.host).unwrap();
