...
impl GpuResourceTracker {
        ...
        if let Some(config) = resources.clone() {
            ...
            if config.mps_enabled() {
                if let Some(docker) = docker.as_any().downcast_ref::<DockerIsolation>() {
                    Self::start_mps(docker, tid).await?;
                } else {
    ...
    async fn start_mps(docker: &DockerIsolation, tid: &TransactionId) -> Result<()> {
        ...
        docker.docker_run(args, img_name, "iluvatar_mps_control", Some(vec!["-f"]), tid, None).await
    }
...
impl Drop for GpuResourceTracker {
    fn drop(&mut self) {
        if let Some(docker) = self.docker.as_any().downcast_ref::<DockerIsolation>() {
            ...
            match Runtime::new() {
                Ok(r) => {
                    match r.block_on(docker.get_logs(MPS_CONTAINER_NAME, tid)) {
