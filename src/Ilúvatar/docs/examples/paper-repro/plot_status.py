...
argparser.add_argument("--trace", '-t', help="Trace experiment was un with", required=True, type=str)
argparser.add_argument("--format", help="The graph format", required=False, type=str, default="png")
argparser.add_argument("--out", help="output folder", required=True, type=str)
argparser.add_argument("--benchmark", help="benchmark file", required=False, type=str, default="../benchmark/worker_function_benchmarks.json")
...
def get_function_map(log_file):
    ...
    with open( log_file, 'r') as f:
        for line in f.readlines():
            if "mapped to function" in line:
                func_name, *parts, mapping = line.split(" ")
                # for p in parts:
                #   if p.startswith("'"):
                #     mapping = p[1:-2]
                # if func_name not in function_code_map:
                function_code_map[func_name]=mapping.strip().strip('\'')
...
fn_map = get_function_map(os.path.join(args.logs_folder, "orchestration.log"))
...
for func_name, group in trace.groupby("func_name"):
  ...
  avg_warm_time = np.mean(bench_data["resource_data"]["cpu"]["warm_results_sec"]) + np.mean(bench_data["resource_data"]["cpu"]["warm_over_results_us"]) / 1_000_000.0
  avg_cold_time = np.mean(bench_data["resource_data"]["cpu"]["cold_results_sec"]) + np.mean(bench_data["resource_data"]["cpu"]["cold_over_results_us"]) / 1_000_000.0
...
ax.stackplot(xs, ldf.status_df["num_running_funcs"].to_numpy(), ldf.status_df["queue_len"].to_numpy(), colors=["tab:cyan", "tab:green"])
ax.hlines(little_sum, min(xs), max(xs), colors=["red"], linestyles=["--"], label=["Little's Law Running"])
