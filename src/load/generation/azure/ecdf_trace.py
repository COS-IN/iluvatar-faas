from dataset import join_day_one, ecdf_trace_row
...
if not (os.path.exists(metadata_save_pth) and os.path.exists(trace_save_pth)) or args.force:
  ...
  for func in functions:
    ...
    for index, row in rows.iterrows():
      map_args.append( (func[:10], row, func[:10], args.duration) )
  ...
  with open(metadata_save_pth, "w") as f:
    ...
    for (function_id, cold_dur, warm_dur, mem) in function_metadata:
      f.write("{},{},{},{}\n".format(function_id, cold_dur, warm_dur, mem))
